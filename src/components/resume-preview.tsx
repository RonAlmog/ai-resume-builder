...
function PersonalInfoHeader({ resumeData }: ResumeSectionProps) {
  ...
  // the goal is to get a photo url.
  useEffect(() => {
    // if it is a file, convert to url.
    const objectUrl = photo instanceof File ? URL.createObjectURL(photo) : "";
    if (objectUrl) setPhotoSrc(objectUrl);
    // if it's null, that means we removed the photo.
    if (photo === null) setPhotoSrc("");

    // clear the object, to avoid memory leak when used over again.
...
function SkillsSection({ resumeData }: ResumeSectionProps) {
  const { skills, colorHex, borderStyle = BorderStyles.CIRCLE } = resumeData;
